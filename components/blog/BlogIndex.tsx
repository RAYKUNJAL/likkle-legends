...
export default function BlogIndex({ posts, categories }: BlogIndexProps) {
    ...
    const categoryName = (post: BlogIndexPost) => categories.find((category) => category.id === post.category)?.name || post.category;
                ...
                <div className="container relative z-10">
                    <div className="max-w-3xl mx-auto text-center">
                        ...
                        <p className="text-xl text-white/80 mb-10 leading-relaxed">
                            Songs, stories, and parent guides for raising Caribbean children at home and in the diaspora.
                            Written for families with kids ages 3–9.
                        </p>
                    ...
                    <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide">
                        ...
                        {categories.map((category) => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-full font-bold text-sm whitespace-nowrap transition-all flex items-center gap-2 ${activeCategory === category.id
                                    ? 'bg-deep text-white shadow-lg'
                                    : 'bg-zinc-100 text-deep/60 hover:bg-zinc-200'
                                    }`}
                            >
                ...
                <div className="container">
                    {posts.length === 0 ? (
                        <div className="text-center py-20">
                            <h2 className="text-2xl font-bold text-deep mb-2">No articles published yet</h2>
                            <p className="text-deep/60">When an article is ready, its title, excerpt, and link will show up here.</p>
                        </div>
                    ) : filteredPosts.length === 0 ? (
                        <div className="text-center py-20">
                            <h2 className="text-2xl font-bold text-deep mb-2">No articles match that search</h2>
                            <p className="text-deep/60">Try another word, or choose All Posts.</p>
                        </div>
                    ) : (
                        <>
                            {featuredPost && (
                                                ...
                                                {post.featured_image_url ? (
                                                    ...
                                                    <div className="w-full h-full bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center">
                                                        <span className="text-5xl opacity-50">
                                                            {categories.find((category) => category.id === post.category)?.icon || '📝'}
                                                        </span>
                                                    </div>
                                                )}
                                            </div>
                ...
                <div className="container">
                    <div className="max-w-3xl mx-auto text-center">
                        ...
                        <p className="text-white/80 text-lg mb-8">
                            Likkle Radio is free to listen to. A parent can also open a Free Forever account for the kid-safe portal. Children do not buy anything inside the app.
                        </p>
                        <Link
                            href="/signup"
                        ...
                        >
                            Parent sign up <ArrowRight size={20} />
                        </Link>
                    </div>
                </div>
            </section>
