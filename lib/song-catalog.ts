...
export type SongCatalogSource = 'owned-asset' | 'suno-cdn' | 'gcs-public' | 'generated-json';
...
/**
 * First-party audio that still exists in this repo and returns HTTP 200
 * on www.likklelegends.com. Do not add invented titles here.
 *
 * Channel mapping matches the live cream landing (#radio) so recovered
 * files stay on the stations parents already hear:
 *   Drinking Water → R.O.T.I Learning Lab
 *   Saving Money   → Tanty Spice Show
 */
export const OWNED_PLAYABLE_SONGS: CatalogSong[] = [
    {
        ...
        status: 'playable',
        source: 'owned-asset',
        notes: 'Owned file at public/assets/youtube/music/drinking-water.mp3 (live 200).',
    },
    {
        ...
        status: 'playable',
        source: 'owned-asset',
        notes: 'Owned file at public/assets/youtube/music/saving-money.mp3 (live 200).',
    },
];

/**
 * Historical catalog recovered from git / seed scripts / live Island Radio.
 * Audio hosts returned 403/404 on 2026-09-19. Inventory only — never listed
 * as a playable row until a first-party file is re-hosted and this status
 * flips to playable.
 */
export const RECOVERED_UNPLAYABLE_SONGS: CatalogSong[] = [
    {
        ...
        source: 'suno-cdn',
        notes: 'Suno CDN 403. No Wayback/GCS/Firebase/sibling-repo master found.',
    },
    {
        id: 'track-roti-2',
    ...
    },
    {
        ...
        status: 'audio_missing',
        source: 'gcs-public',
        notes: 'GCS 404 — title recovered from scripts/seed-radio.ts only.',
    },
    {
        id: 'gcs-anansi-story',
...
const OWNED_URL_PREFIXES = ['/assets/youtube/music/', '/audio/'];
...
export function getPlayableChannelIds(): string[] {
    return [...new Set(getPlayableTracks().map((t) => t.channel).filter(Boolean))] as string[];
}

export function isOwnedPlayableUrl(url?: string | null): boolean {
    if (!url) return false;
    if (OWNED_URL_PREFIXES.some((prefix) => url.startsWith(prefix))) return true;
    return getPlayableCatalogSongs().some((song) => song.url === url);
}

export function filterPlayableTracks(tracks: Track[]): Track[] {
    return tracks.filter((track) => isOwnedPlayableUrl(track.url));
}
